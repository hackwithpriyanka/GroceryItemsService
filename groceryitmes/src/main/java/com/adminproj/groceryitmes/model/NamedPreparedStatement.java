...
public class NamedPreparedStatement extends PreparedStatementImpl{
	...
	private static Logger log = LoggerFactory.getLogger(NamedPreparedStatement.class);
  ...
  public static NamedPreparedStatement prepareStatement(Connection conn, String sql) throws SQLException {
      ...
      log.info("NamedPreparedStatement-->prepareStatement-->Query"+parsedQuery.toString());
      return new NamedPreparedStatement(conn.prepareStatement(parsedQuery.toString()), sql, orderedParameters);
  }
 
  public static NamedPreparedStatement prepareStatement(Connection conn, String sql,int startRow,int endRow) throws SQLException {
      List<String> orderedParameters = new ArrayList<String>();
      ...
      dataQuery=parsedQuery.toString();
      countQuery=parsedQuery.toString();
      if(endRow>0)
      {
      	countQuery="select count(*) as count from ("+dataQuery+") as countTable";
      	dataQuery=dataQuery.toLowerCase().replaceFirst("select", "select row_number() over (order by 1 desc) as rownum,");
      	dataQuery="select * from ("+dataQuery+") as mainData where rownum between "+startRow+ " and "+endRow;        	    	
      	 log.info("NamedPreparedStatement-->prepareStatement-->dataQuery"+dataQuery.toString());
      	 log.info("NamedPreparedStatement-->prepareStatement-->countQuery"+countQuery.toString());
      	return new NamedPreparedStatement(conn.prepareStatement(dataQuery),conn.prepareStatement(countQuery), sql, orderedParameters);
      }
      else
      {
      	 return new NamedPreparedStatement(conn.prepareStatement(dataQuery), sql, orderedParameters);
      }
  ...
  private NamedPreparedStatement(PreparedStatement preparedStatement,PreparedStatement countPreparedStatement, String originalSQL, List<String> orderedParameters) {
      super(preparedStatement,countPreparedStatement);
      this.originalSQL = originalSQL.trim();
      this.lstParameters = orderedParameters;
  }
  ...
  private Collection<Integer> getParameterIndexes(String parameter) {
	...
	public void setParameter(Parameter parameter) throws SQLException
  {
  	ArrayList<Parameter> parameterList=parameter.getParameterList();
  	if(parameterList!=null)
  	{
  	for (Parameter parameterObj : parameterList) {
		  if(parameterObj.getParameterType()==PARAMETER_TYPE.STRING)
		  {
	      	this.setString(parameterObj.getParameterName(),parameterObj.getParameterValue().toString());		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.DOUBLE)
		  {
	      	this.setDouble(parameterObj.getParameterName(),Double.parseDouble(parameterObj.getParameterValue().toString()));		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.LONG)
		  {
	      	this.setLong(parameterObj.getParameterName(),Long.parseLong(parameterObj.getParameterValue().toString()));		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.INT)
		  {
	      	this.setInt(parameterObj.getParameterName(),Integer.parseInt(parameterObj.getParameterValue().toString()));		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.DATE)
        {
            this.setDate(parameterObj.getParameterName(),(parameterObj.getParameterValue()==null || parameterObj.getParameterValue().equals(""))?null:new java.sql.Date( ((java.util.Date)parameterObj.getParameterValue()).getTime()));        
        }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.SQLDATE)
		  {
	      	this.setDate(parameterObj.getParameterName(),(java.sql.Date)(parameterObj.getParameterValue()));		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.SQLTIMESTAMP)
		  {
	      	this.setTimestamp(parameterObj.getParameterName(),(parameterObj.getParameterValue()==null || parameterObj.getParameterValue().equals(""))?null:(java.sql.Timestamp)(parameterObj.getParameterValue()));		
		  }
		  else if(parameterObj.getParameterType()==PARAMETER_TYPE.BIGDECIMAL)
		  {
			  this.setBigDecimal(parameterObj.getParameterName(),new BigDecimal(parameterObj.getParameterValue().toString()));
	      	
			  //this.setb(parameterObj.getParameterName(),!CommonFunction.isNeitherNullNorEmpty(parameterObj.getParameterValue())?BigDecimal.ZERO:(parameterObj.getParameterValue()));		
		  }
		}
  	}
  }
  ...
  public void setStringWithCount(String parameter, String x) throws SQLException {
      for (Integer i : getParameterIndexes(parameter)) {
          getPreparedStatement().setString(i, x);
          getCountPreparedStatement().setString(i, x);
          this.originalSQL = this.originalSQL.replaceFirst("(?i):" + parameter, Matcher.quoteReplacement(format((String) x, FormatType.STRING)));
      }
  }
  ...
  public void setDateWithCount(String parameter, Date x) throws SQLException {
      for (Integer i : getParameterIndexes(parameter)) {
          getPreparedStatement().setDate(i, x);
          getCountPreparedStatement().setDate(i, x);
          this.originalSQL = this.originalSQL.replaceFirst("(?i):" + parameter, Matcher.quoteReplacement(format((Date) x, FormatType.DATE)));
      }
  }
